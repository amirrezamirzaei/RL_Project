...
def parse_args():
    ...
    parser.add_argument("--total-timesteps", type=int, default=int(500_000),
...
if __name__ == "__main__":
    ...
    # env setup
    # envs = JumpTaskEnvWithColors(scr_w=64, scr_h=64)
    envs = gym.make('jumping-colors-task-v0', scr_w=64, scr_h=64)
    # envs = gym.wrappers.TransformObservation(envs, lambda obs: obs["rgb"])
    envs.single_action_space = envs.action_space
    # envs.single_observation_space = envs.observation_space["rgb"] todo why
    ...
    if args.agent=='normal':
        ...
        agent = AgentClip(envs, clip_model, clip_processor, "white rectangle near green square but not touching it.").to(device)
    ...
    next_obs = torch.Tensor(state.copy()).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs, next_done = torch.Tensor(next_obs.copy()).to(device), torch.Tensor([done]).to(device)
