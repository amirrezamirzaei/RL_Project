...
class AgentClip(nn.Module):
    ...
    def get_value(self, x):
        ...
        hidden = self.network(x.reshape((-1,64,64,3)).permute((0, 3, 1, 2)) / 255.0)  # "bhwc" -> "bchw" batch_size * 256
...
class AgentNormal(nn.Module):
    ...
    def get_value(self, x):
        return self.critic(self.network(x.reshape((-1,64,64,3)).permute((0, 3, 1, 2)) / 255.0))  # "bhwc" -> "bchw"

    def get_action_and_value(self, x, action=None):
        hidden = self.network(x.reshape((-1,64,64,3)).permute((0, 3, 1, 2)) / 255.0)  # "bhwc" -> "bchw"
        logits = self.actor(hidden)
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
